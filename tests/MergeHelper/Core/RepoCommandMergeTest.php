...
class MergeHelper_Core_RepoCommandMergeTest extends PHPUnit_Framework_TestCase {
	...
	public function test_getMergeCommandForRevision() {
		$oMergeCommand = new MergeHelper_Core_RepoCommandMerge($this->oRepo, new MergeHelper_Core_CommandLineBuilder());
		$oMergeCommand->setRevision(new MergeHelper_Core_Revision('5'));
		$oMergeCommand->setRepoPath(new MergeHelper_Core_RepoPath('/branches/platform/_production/2010-01-04'));
		$oMergeCommand->setWorkingCopyPath('/var/tmp/testwc');
		$sCommandline = $oMergeCommand->sGetCommandline();

		$this->assertSame('svn merge -c 5 file://'.realpath(MergeHelper_Helper_Bootstrap::sGetPackageRoot().'/../tests/_testrepo').'/branches/platform/_production/2010-01-04 /var/tmp/testwc',
		                  $sCommandline);
	}
	...
	public function test_getMergeCommandForRevisionDryRun() {
		...
		$this->assertSame('svn merge --dry-run -c 5 file://'.realpath(MergeHelper_Helper_Bootstrap::sGetPackageRoot().'/../tests/_testrepo').'/branches/platform/_production/2010-01-04 /var/tmp/testwc',
		                  $sCommandline);
	}
